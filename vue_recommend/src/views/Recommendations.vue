      ...
      <div v-for="(movie, index) in recommendedMovies" :key="movie.id" class="movie-card">
        ...
        <div class="movie-content">
          ...
          <div class="movie-meta">
            <p class="movie-director"><strong>导演:</strong> {{ movie.director || '未知' }}</p>
            <p class="movie-year"><strong>年份:</strong> {{ movie.year || movie.release_year || '未知' }}</p>
            <p class="movie-genre"><strong>类型:</strong> {{ formatGenres(movie.genres) }}</p>
            <p class="movie-actors" v-if="movie.actors && movie.actors.length > 0">
              <strong>主演:</strong> {{ formatActors(movie.actors) }}
            </p>
          </div>
