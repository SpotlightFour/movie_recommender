...
graph TB
    ...
    subgraph 汇总层 [汇总与调整层]
        ...
        M --> N[🧮 计算原始总分]
        N --> O[⚖️ 范围调整<br>1.0-5.0]
        O --> P[⭐ 最终预测评分<br>4.2/5.0]
    end
    ...
    %% 样式定义
    classDef inputLayer fill:#e1f5fe,stroke:#0277bd,stroke-width:3px,stroke-dasharray: 3 3
    classDef calculationLayer fill:#f3e5f5,stroke:#7b1fa2,stroke-width:3px
    classDef aggregationLayer fill:#fff3e0,stroke:#ff8f00,stroke-width:3px
    classDef outputLayer fill:#e8f5e9,stroke:#2e7d32,stroke-width:3px,stroke-dasharray: 3 3
    
    %% 节点样式
    classDef startNode fill:#bbdefb,stroke:#1565c0,stroke-width:2px
    classDef featureNode fill:#b3e5fc,stroke:#0277bd,stroke-width:1.5px
    classDef analysisNode fill:#81d4fa,stroke:#01579b,stroke-width:2px
    classDef match1 fill:#f8bbd0,stroke:#c2185b,stroke-width:1.5px
    classDef match2 fill:#c5cae9,stroke:#303f9f,stroke-width:1.5px
    classDef match3 fill:#c8e6c9,stroke:#388e3c,stroke-width:1.5px
    classDef match4 fill:#fff9c4,stroke:#f57f17,stroke-width:1.5px
    classDef score1 fill:#f48fb1,stroke:#c2185b,stroke-width:1.5px
    classDef score2 fill:#9fa8da,stroke:#3949ab,stroke-width:1.5px
    classDef score3 fill:#a5d6a7,stroke:#388e3c,stroke-width:1.5px
    classDef score4 fill:#ffecb3,stroke:#ffa000,stroke-width:1.5px
    classDef aggregationNode fill:#ffcc80,stroke:#ef6c00,stroke-width:2px
    classDef resultNode fill:#fff59d,stroke:#f9a825,stroke-width:2px
    classDef finalNode fill:#a5d6a7,stroke:#388e3c,stroke-width:2px
    
    %% 应用大框样式
    class 输入层 inputLayer
    class 计算层 calculationLayer
    class 汇总层 aggregationLayer
    class 输出层 outputLayer
    
    %% 应用节点样式
    class A startNode
    class B,C featureNode
    class D analysisNode
    class E match1
    class F match2
    class G match3
    class H match4
...
class ExplanationAgent:
    ...
    def evaluate_explanation(self, user_id: int, movie_title: str, explanation: str,
        ...
        try:
            ...
            # 文本长度评估
            length_metrics = self._evaluate_text_length(explanation)
    ...
    def _evaluate_text_length(self, explanation: str) -> Dict[str, Any]:
        """
        评估文本长度指标
        """
        try:
            ...
            # 平均句子长度（单词/句子）
            avg_sentence_length = char_count / sentence_count if sentence_count > 0 else char_count
            ...
            if explanation_type == 2:
                ...
                if movie_info:
                    ...
                    context_items.extend([
                        ...
                        f"评分：{movie_info.get('vote_average', 0)}"
                    ])
                ...
                if movie_obj and user_profile:
                    ...
                    context_items.extend([
                        f"类型偏好得分：{match_scores.get('genre_bonus', 0):.2f}",
                        f"导演偏好得分：{match_scores.get('director_bonus', 0):.2f}",
                        f"演员偏好得分：{match_scores.get('actor_bonus', 0):.2f}",
                    ])
                ...
                if movie_info:
                    context_items.extend([
                        ...
                        f"类型：{', '.join(movie_info.get('genres', []))}",
                        f"评分：{movie_info.get('vote_average', 0)}/5"
                    ])
            elif explanation_type == 3:  # How 过程解释
                # 获取匹配分数用于构建流程相关的上下文
                match_scores = self._calculate_match_scores(user_id, movie_info)
                context_items.extend([
                    f"特征分析",
                    f"提取电影《{movie_info.get('title', '未知')}》的特征信息",
                    f"获取用户偏好: {', '.join(preferences[:2])}",
                    f"类型得分: {match_scores.get('genre_bonus'):.2f}/3.0",
                    f"导演得分: {match_scores.get('director_bonus'):.2f}/0.8",
                    f"演员得分: {match_scores.get('actor_bonus'):.2f}/1.0",
                    f"计算原始总分: {match_scores.get('total_score'):.2f}",
                    f"范围调整：1.0-5.0",
                    f"最终预测评分: {match_scores.get('predicted_rating'):.2f}"
                ])
    ...
    def _get_evaluation_causal_factors(self, user_id: int, movie_title: str, explanation_type: int = None) -> List[str]:
        ...
        try:
            ...
            if explanation_type == 2:
                ...
                causal_factors.extend([
                    f"特征提取是评分计算的一步",
                    f"类型得分{match_scores.get('genre_bonus'):.2f}影响总分",
                    f"导演得分{match_scores.get('director_bonus'):.2f}影响总分",
                    f"演员得分{match_scores.get('actor_bonus'):.2f}影响总分",
                    f"各维度得分累加得到原始总分{match_scores.get('total_score'):.2f}",
                    f"范围调整确保评分在1.0-5.0分区间",
                ])
