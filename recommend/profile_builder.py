...
class ProfileBuilder:
    ...
    def analyze_user_behavior(self, user_id):
        ...
        try:
            ...
            if profile is not None:
                # 只有 profile 不为 None 时才访问其属性
                analysis['watch_time_pattern'] = profile.watch_time_pattern or ''
                print(f"✅ 从现有画像获取观看时间模式: {analysis['watch_time_pattern']}")
            else:
                print(f"ℹ️ 用户 {user_id} 没有现有画像，使用默认观看时间模式")
                analysis['watch_time_pattern'] = ''
            if explicit_preferences:
                ...
                # 2. 如果没有显式偏好，从评分数据中学习
                print("🎬 从评分数据学习偏好")
                learned_genres = self._learn_genres_from_behavior(user_id)
                rating_based_prefs = self._learn_from_action(user_id)
                analysis.update(rating_based_prefs)
                analysis['genres'] = learned_genres  # 使用专门学习的类型偏好
    ...
    def _learn_genres_from_behavior(self, user_id):
        """从用户行为中专门学习类型偏好（0-5分）"""
        print(f"🎯 开始为用户 {user_id} 学习类型偏好")
        ...
        try:
            ...
            print(f"📈 分析 {len(ratings)} 个评分中的类型偏好")
            ...
            print(f"❤️ 分析 {len(likes)} 个点赞中的类型偏好")
            print(f"🔖 分析 {len(bookmarks)} 个收藏中的类型偏好")
            print(f"👀 分析 {len(views)} 个浏览中的类型偏好")
            ...
            print(f"🔍 分析 {len(frequently_viewed_movies)} 个频繁浏览的电影中的类型偏好")
            ...
            for rating in ratings:
                ...
                self._process_movie_genres(movie, weight, genre_scores)
            ...
            # 打印原始分数
            print(f"🎬 类型原始分数: {dict(list(genre_scores.items())[:5]) if genre_scores else '无数据'}")

            # 归一化处理：将分数转换为0-5分范围
            def normalize_genre_scores(scores_dict):
                if not scores_dict:
                    return {}
                ...
                return dict(sorted_items[:10])  # 只保留前10个类型

            normalized_genres = normalize_genre_scores(genre_scores)
            print(f"✅ 学习到的类型偏好: {normalized_genres}")

            return normalized_genres

        except Exception as e:
            print(f"❌ 从行为学习类型偏好失败: {e}")
            import traceback
            traceback.print_exc()
            return {}

    def _process_movie_genres(self, movie, weight, genre_scores):
        """处理单部电影的类型并累加到分数中"""
        if movie.genres:
            # 假设 genres 是以逗号分隔的字符串
