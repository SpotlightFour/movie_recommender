...
class ImprovedIMDbClient:
    ...
    def format_movie_info(self, imdb_data, movie_title):
        """格式化IMDb数据为统一格式"""
        if not imdb_data:
            return None
        ...
        imdb_actors = imdb_data.get('actors', [])

        # 初始化变量
        director_str = "未知导演"
        actors_list = []

        # 如果IMDb返回的导演信息不为空，使用IMDb的数据
        if imdb_directors and len(imdb_directors) > 0:
            director_str = ', '.join(imdb_directors)
            print(f"使用IMDb导演信息: {director_str}")
        else:
            # IMDb导演信息为空，尝试从数据库获取
            print(f"IMDb导演信息为空，尝试从数据库获取")

            try:
                movie = Movie.query.filter(Movie.title.ilike(f"%{movie_title}%")).first()
                director_str = movie.director
                print(f"从数据库获取导演信息: {director_str}")
            except Exception as e:
                print(f"数据库查询导演失败: {str(e)}")

        # 如果IMDb返回的演员信息不为空，使用IMDb的数据
        if imdb_actors and len(imdb_actors) > 0:
            actors_list = imdb_actors
            print(f"使用IMDb演员信息: {actors_list}")
        else:
            # IMDb演员信息为空，尝试从数据库获取
            print(f"IMDb演员信息为空，尝试从数据库获取")

            try:
                movie = Movie.query.filter(Movie.title.ilike(f"%{movie_title}%")).first()
                if movie and hasattr(movie, 'actors') and movie.actors:
                    actors_list = movie.actors.split(',')
                    print(f"从数据库获取演员信息: {actors_list}")
                else:
                    print(f"数据库中未找到电影或演员信息: {movie_title}")
            except Exception as e:
                print(f"数据库查询演员失败: {str(e)}")

        # 获取IMDb评分并进行归一化（IMDb是10分制，转为5分制）
        imdb_rating = imdb_data.get('vote_average', 0)
        normalized_rating = imdb_rating / 2.0  # 从10分制转为5分制
        ...
        return {
            ...
            'poster_url': imdb_data.get('poster_url', ''),
            'actors': actors_list,
            'vote_average': round(normalized_rating, 1),  # 保留一位小数
            'vote_count': imdb_data.get('vote_count', 0),
            'runtime': imdb_data.get('runtime', 0),
            'source': 'imdb'
        }
...
class ExplanationAgent:
    ...
    def _search_movie_online(self, movie_title):
        ...
        try:
            ...
            if movie_data:
                formatted_data = self.imdb_client.format_movie_info(movie_data, movie_title)
